...
This is a modified version of the pcalg package using a conditional independence test
from the pingouin package.
...
def partial_corr_mat(mat,i1,i2,conds):
    '''
    Returns the p-value of a partial correlation between variables in a matric
    Pingouin's API requies us to transform the input matrix into a dataframe. 
    ...
    pc = pg.partial_corr(df,str(i1),str(i2),list([str(x) for x in conds]))['p-val']
    
    return(float(pc))

def plot_graphs(g,label_names,layout,nodec='orange',labelc='white'):
    '''
    Function to plot graphs in a set layout, and add node labels and colors
    
    Args:
        g (nx object) is the graph
        layout (nx layout) is the layout method
        label_names (list) is the list of names for the node labels
        nodec (str) is the color for the nodes
        fontc (str) is the color for the font in the nodes
    '''
    ...
    #Draw the network
    nx.draw(g,pos,node_color=nodec)
    
    #Draw the labels
    nx.draw_networkx_labels(
        g,pos,labels={n:x for n,x in enumerate(label_names)},font_color=labelc)
